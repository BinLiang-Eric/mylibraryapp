...
const getAllAuthors = async (req, res) => {
  try {
    ...
    return res.render('authorlist', {'authors':authors})
  } catch (err) {
    res.status(400)
...
const getOneAuthor = async (req, res) => {  
    try {
        const oneAuthor = await Author.findOne( {"authorId": req.params.authorId} ).lean()
        if (oneAuthor === null) {   // no author found in database
            res.status(404)
            return res.send("Author not found").lean()
        }
        return res.render('oneAuthor', {'thisAuthor': oneAuthor})  // author was found
...
const updateAuthor = async (req, res) => {
  ...
  try {
    ...
    const oneAuthor = await Author.findOne( {"authorId": req.body.authorId} ).lean()
    res.render('oneAuthor', {'thisAuthor':oneAuthor})             // return saved author to sender
